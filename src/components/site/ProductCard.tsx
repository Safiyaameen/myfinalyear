...
export function ProductCard({ product }: { product: any }) {
  // Support both Supabase format (price, image_hue) and old format (offers, imageHue)
  const isSupabase = !product.offers;
...
/>
        <div className="p-4">
          ...
          <div className="mt-3 flex items-end justify-between gap-2">
            <div>
              <div className="font-display text-lg font-semibold">{formatLKR(Number(product.price))}</div>
              <div className="text-xs text-muted-foreground">
                {product.stock} in stock · {product.delivery}
              </div>
            </div>
          </div>
        </div>
  ...
  // Old hardcoded format (still works for homepage etc)
  const cheapest = product.offers.reduce((a: any, b: any) => (a.price < b.price ? a : b));
  return (
    <Link
      to="/product/$slug"
      params={{ slug: product.slug }}
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-end justify-between gap-2">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              from {cheapest.shopSlug} · {product.offers.length} {product.offers.length === 1 ? "shop" : "shops"}
            </div>
          </div>
