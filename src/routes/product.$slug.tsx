...
function ProductPage() {
  ...
  // Find all shops selling the same product (matched by title)
  const { data: offers = [] } = useQuery({
    queryKey: ["product-offers", p.title],
    queryFn: async () => {
  ...
  const sorted = offers.length ? offers : [p];
        ...
        <div>
          ...
          <div className="mt-6 rounded-2xl border border-gold/40 bg-gold/5 p-5">
            ...
            <div className="mt-1 flex items-end justify-between gap-4">
              <div className="font-display text-4xl font-semibold">{formatLKR(Number(cheapest.price))}</div>
              <div className="text-right text-sm text-muted-foreground">
                from <span className="font-medium text-foreground">{cheapest.shops?.name}</span>
                <br />
                {cheapest.delivery}
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                onClick={() => {
                  addToCart(cheapest.slug, cheapest.shops?.slug);
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right">
                    <div className="font-display text-lg font-semibold">{formatLKR(Number(o.price))}</div>
                    <button
                      onClick={() => {
                        addToCart(o.slug, shop.slug);
                        toast.success("Added to cart");
                      }}
